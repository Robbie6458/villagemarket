...
export default async function BarterPage() {
  ...
  return (
    <div className="min-h-screen bg-lamp">
      {/* Hero */}
      <section className="bg-lamp border-b border-fence">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-14 md:py-18">
          <p className="text-gold text-sm font-medium tracking-widest uppercase mb-3">Community Exchange</p>
          <h1
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={step} className="bg-linen rounded-2xl p-5">
        ...
        <section>
          ...
          {barterSellers.length > 0 ? (
            <div className="space-y-3">
              {barterSellers.map((seller) => (
                <Link
                  ...
                  className="group flex items-start gap-4 bg-linen rounded-2xl p-5 hover:shadow-md transition-all hover:-translate-y-0.5"
                >
                  <div className="w-10 h-10 bg-gold/15 rounded-full flex items-center justify-center shrink-0 text-lg">
                    🤝
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <h3 className="font-medium text-bark group-hover:text-flamelo transition-colors text-sm">
                        {seller.name}
                      </h3>
                      <span className="text-[10px] text-gold bg-lamp px-2 py-0.5 rounded-full font-medium shrink-0">
                  ...
                  <span className="text-bark/30 group-hover:text-flamelo transition-colors text-sm shrink-0 mt-0.5">
                    →
                  </span>
                </Link>
              ))}
            ...
            <div className="bg-linen rounded-2xl p-10 text-center text-bark/40">
        ...
        {/* Are you a seller? */}
        <section className="relative bg-ember rounded-2xl p-8 overflow-hidden">
          <div className="absolute inset-0 ember-glow opacity-60" />
          <div className="relative">
            <p className="text-gold text-xs font-medium tracking-[0.2em] uppercase mb-2" style={{ fontFamily: "var(--font-mono)" }}>Sellers</p>
            <h2 className="font-display text-2xl text-linen mb-3">
              Open to trading your work?
            </h2>
            <p className="text-linen/65 text-sm leading-relaxed mb-5 max-w-lg">
            ...
            <Link
              href="/dashboard"
              className="inline-block bg-gold text-ember text-sm font-semibold px-5 py-2.5 rounded-full hover:bg-goldsoft transition-colors"
            >
        ...
        <section className="bg-lamp rounded-2xl p-8">
          <p className="text-gold text-xs font-medium tracking-widest uppercase mb-2">Coming Soon</p>
